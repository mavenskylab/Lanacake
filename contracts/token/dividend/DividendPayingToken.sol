    ...
    IDividendPayingTokenOptional
{
    ...
    /// It emits the `DividendsDistributed` event if the amount of received bnb is greater than 0.
    /// About undistributed bnb:
    /// In each distribution, there is a small amount of bnb not distributed,
    /// the magnified amount of which is
    /// `(msg.value * magnitude) % totalSupply()`.
    /// With a well-chosen `magnitude`, the amount of undistributed bnb
    /// (de-magnified) in a distribution can be less than 1 wei.
    /// We can actually keep track of the undistributed bnb in a distribution
    /// and try to distribute it in the next distribution,
    /// but keeping track of such data on-chain costs much more than
    /// the saved bnb, so we don't do that.
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success = IBEP20(dividendToken).transfer(
                user,
                _withdrawableDividend
            );
